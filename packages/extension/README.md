...
注意这会把当前 **15 个工具**的 schema **常驻上下文**：实测 name + description + inputSchema
约 7.6k 字符，客户端若同时展开 outputSchema 则约 15k 字符。
...
| 路径 | 作用 |
|---|---|
| `public/manifest.json` | MV3 清单，权限与入口（不经打包，原样拷进 dist） |
| `public/popup.html` | popup 界面（同上） |
| `src/background/index.ts` | SW 入口：建连接、分发请求、闹钟兜底 |
| `src/background/connection.ts` | WebSocket 生命周期、判活、退避重连 |
| `src/background/dispatch.ts` | 动作分发表，与 `ACTIONS` 一一对应 |
| `src/background/ax.ts` | 无障碍树 → 缩进快照 |
| `src/background/cdp.ts` | 调试器附加、可见性过滤、坐标点击 |
| `src/background/inject.ts` | 注入页面的自包含函数 |
| `src/background/network.ts` | 网络录制与环形缓冲区 |
| `src/background/refs.ts` | ref ↔ DOM 节点映射表 |
| `src/background/diag.ts` | 跨 SW 存活的诊断日志 |
| `src/background/settings.ts` | 服务地址与令牌 |
| `src/background/badge.ts` | 徽标状态 |

仓库根的 `CLAUDE.md` 记了更多"为什么这么写"的设计约束，改代码前值得先读。
